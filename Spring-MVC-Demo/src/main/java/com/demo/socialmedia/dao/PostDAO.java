...
public class PostDAO {
    ...
    private final RowMapper<Post> rowMapper = (rs, rowNum) -> {
        ...
        try {
            ...
            post.setLikeCount(0); // Fallback if column doesn't exist yet
        }
        return post;
    };
    ...
    public List<Post> findFeedByUserId(int userId) {
        return findFeedWithPagination(userId, 10, 0);
    }

    // Lấy newsfeed có phân trang và ưu tiên
    public List<Post> findFeedWithPagination(int userId, int limit, int offset) {
        // Thuật toán ưu tiên: Bài viết của người mình follow + sắp xếp theo like_count và thời gian
        String sql = "SELECT p.*, u.username FROM posts p "
                   + "JOIN users u ON p.user_id = u.id "
                   + "WHERE p.user_id IN ("
                   ...
                   + "LIMIT ? OFFSET ?";
        return jdbcTemplate.query(sql, rowMapper, userId, userId, limit, offset);
    }
