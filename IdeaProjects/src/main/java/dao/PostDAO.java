...
public class PostDAO {
        ...
        try (Connection conn = Database.getConnection();
            ...
            while (rs.next()) {
                posts.add(new Post(rs.getInt("id"), rs.getString("title"),
                                  ...
                                  rs.getInt("like_count"), rs.getString("author_name")));
            }
    ...
    public List<Post> getAdvancedFeed(int userId, int limit, int offset) {
        List<Post> posts = new ArrayList<>();
        String sql =
                ...
                "WHERE p.user_id = ? " +
                "   OR (p.status = 'published' AND p.user_id IN (SELECT followed_user_id FROM follows WHERE following_user_id = ?)) " +
                "GROUP BY p.id " +
